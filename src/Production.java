...
public class Production {
    ...
    public ArrayList<Integer> computeFirstSetOfBetaAndC() {
    	...
    	for (int i = 0; i < set.size(); i++) {
    		firstSet = manager.getFirstSetBuilder().getFirstSet(set.get(i));
    ...
    public void productionMerge(Production product) {
    	System.out.println("Production merge");
    	System.out.println("old product: ");
    	this.print();
    	System.out.println("new product: ");
    	product.print();
    	
    	if (this.productionEequals(product)) {
    		for (int i = 0; i < product.lookAhead.size(); i++) {
    			if (this.lookAhead.contains(product.lookAhead.get(i)) == false) {
    				this.lookAhead.add(product.lookAhead.get(i));
    			}
    		}
    	}
    }
